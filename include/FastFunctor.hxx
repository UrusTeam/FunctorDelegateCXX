...
//  Documentation is found at http://www.codeproject.com/KB/cpp/FastFunctor.aspx
//
...
//				  * Added FastFunctor::empty() to test if invocation is safe (Thanks Neville Franks).
...
//				  * New syntax: FastFunctor< int (char *, double) >. 
...
@VARARGS
template<class RetType=detail::DefaultVoid, @CLASSARGS>
class FastFunctor@NUM {
private:
...
public:
	...
	typedef FastFunctor@NUM type;

	// Construction and comparison functions
	FastFunctor@NUM() { clear(); }
	FastFunctor@NUM(const FastFunctor@NUM &x) {
		m_Closure.CopyFrom(this, x.m_Closure); }
	void operator = (const FastFunctor@NUM &x)  {
		m_Closure.CopyFrom(this, x.m_Closure); }
	bool operator ==(const FastFunctor@NUM &x) const {
	...
	FastFunctor@NUM(Y *pthis, DesiredRetType (X::* function_to_bind)(@FUNCARGS) ) {
	...
	template < class X, class Y >
	FastFunctor@NUM(const Y *pthis, DesiredRetType (X::* function_to_bind)(@FUNCARGS) const) {
	...
	FastFunctor@NUM(DesiredRetType (*function_to_bind)(@FUNCARGS) ) {
	...
	inline void bind(DesiredRetType (*function_to_bind)(@FUNCARGS)) {
		m_Closure.bindstaticfunc(this, &FastFunctor@NUM::InvokeStaticFunction, 
			function_to_bind); }
	// Invoke the delegate
	RetType operator() (@FUNCARGS @INVOKEARGS) const {
...
private:	// Invoker for static functions
	RetType InvokeStaticFunction(@FUNCARGS @INVOKEARGS) const {
...
// 
//				FastFunctor<> class (Original author: Jody Hagins)
//	Allows boost::function style syntax like:
//			FastFunctor< double (int, long) >
// instead of:
...
// Declare FastFunctor as a class template.  It will be specialized
...
// FastFunctor< R ( @SELARGS ) >
// instead of 
// FastFunctor@NUM < @SELARGS, R >
template<typename R, @CLASSARGS>
class FastFunctor< R ( @SELARGS ) >
  // Inherit from FastFunctor@NUM so that it can be treated just like a FastFunctor@NUM
  : public FastFunctor@NUM < R, @SELARGS >
{
public:
  // Make using the base type a bit easier via typedef.
  typedef FastFunctor@NUM < R , @SELARGS > BaseType;

  // Allow users access to the specific type of this delegate.
  typedef FastFunctor SelfType;

  // Mimic the base class constructors.
  FastFunctor() : BaseType() { }
  ...
  template < class X, class Y >
  FastFunctor(const Y *pthis,
      R (X::* function_to_bind)( @FUNCARGS ) const)
    : BaseType(pthis, function_to_bind)
  {  }

  FastFunctor(R (*function_to_bind)( @FUNCARGS ))
...
//
//			MakeDelegate(&x, &X::func) returns a fastfunctor of the type
...
FastFunctor@NUM<FASTDLGT_RETTYPE, @SELARGS> MakeDelegate(Y* x, RetType (X::*func)(@FUNCARGS)) { 
	return FastFunctor@NUM<FASTDLGT_RETTYPE, @SELARGS>(x, func);
}

template <class X, class Y, class RetType, @CLASSARGS>
FastFunctor@NUM<FASTDLGT_RETTYPE, @SELARGS> MakeDelegate(Y* x, RetType (X::*func)(@FUNCARGS) const) { 
	return FastFunctor@NUM<FASTDLGT_RETTYPE, @SELARGS>(x, func);
}
