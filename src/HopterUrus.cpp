...
*/
#if NUMARGS < 1
#error "Provide a count argument > 0"
#endif

#define NUMARGSN {NUMARGS - 1}

const int MaxNumArgs = NUMARGSN;
...
void PrintVarArgs(FILE *fout, CString bigblock, int num) {
	...
	if (num==0) {
		...
		for (int k=2; k<=num; k++) {
			str.Format(", p%d", k);
			invokelist+=str;
			str.Format(", Params%d p%d", k, k);
			funclist+=str;
			str.Format(", Params%d", k);
			selectlist += str;
			str.Format(", class Params%d", k);
			classlist += str;
		}
	}
